...
def render_display_records(request, **kwargs):
    def get_unique_values(records, model_class, field):
        if model_class == StaffData:
            ...
            # remove None values
            contain_data_unique_values = [
                value for value in contain_data_unique_values if value != None
            ]
        ...
        contain_data_unique_values = [
            value for value in contain_data_unique_values if value in unique_values
        ]
        ...
        if contain_data_unique_values:
            unique_values = contain_data_unique_values + [
                value
                for value in unique_values
                if value not in contain_data_unique_values
            ]
        ...
        if keyword:
            ...
            unique_values = [
                value for value in unique_values if keyword.lower() in value.lower()
            ]
        ...
        if model_class == VehicleDetail:
            ...
            unique_values = [
                value for value in unique_values if not value.startswith("72")
            ]
    ...
    if model_class == Job:
        for record in records:
            ...
            record.progress_by_amount = progress_by_amount(
...
def render_message(request, message, **kwargs):
    ...
    context = {
        ...
        "ok_button_function": ok_button_function,
    }
...
def render_weekplan_table(request, project_id, check_date=None):
    ...
    jobplans_in_week = JobPlan.objects.filter(
        start_date__gte=monday, end_date__lte=sunday, job__project=project
    )
    job_date_reports = JobDateReport.objects.filter(
        date=check_date, job__project=project
    )

    jobs = Job.objects.filter(project=project)

    for job in jobs:
        job.progress_left_by_quantity_upto_lastweek = (
            job.quantity
            - progress_by_quantity(job, check_date=last_sunday)[
    ...
    for jobplan in jobplans_in_week:
        # Get date report
        job_date_report = job_date_reports.filter(
            job=jobplan.job, date=check_date
        ).first()
        if job_date_report:
    ...
    for jobplan_in_week in jobplans_in_week:
        jobplan_in_week.progress_by_time = progress_by_time(
            jobplan_in_week, check_date=check_date
        )
        jobplan_in_week.progress_by_amount = progress_by_amount(
            jobplan_in_week, check_date=check_date
        )

    template = "components/weekplan_table.html"
    context = {
        "jobplans_in_week": jobplans_in_week,
