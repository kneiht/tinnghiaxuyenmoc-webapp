...
class Job(SecondaryIDMixin, BaseModel):
    ...
    name = models.CharField(max_length=2000, verbose_name="Tên công việc")
    category = models.CharField(
        max_length=2000, default="Chưa phân loại", verbose_name="Danh mục"
    )
    ...
    unit_price = models.FloatField(
        default=0.0, validators=[MinValueValidator(0)], verbose_name="Đơn giá"
    )
    total_amount = models.FloatField(
        default=0.0, validators=[MinValueValidator(0)], verbose_name="Thành tiền"
    )
    ...
    def clean(self):
        ...
        if not self._state.adding:  # Only check for existing jobs being modified
            ...
            if (has_plans or has_reports) and (
                original_job.name != self.name
                or original_job.unit != self.unit
                or original_job.unit_price != self.unit_price
                or original_job.quantity != self.quantity
                or original_job.category != self.category
            ):
        ...
        if self._state.adding:
            if Job.objects.filter(
                project=self.project, name=self.name, category=self.category
            ).exists():
