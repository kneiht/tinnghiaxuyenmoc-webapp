...
def download_excel(request, model_name):
    ...
    try:
        ...
        for field_name in fields:
            ...
            elif field.is_relation and field.many_to_one:  # This checks if it's a ForeignKey
                # Get the related model
                related_model = field.related_model
                # Create a dictionary of id to string representation
                related_objects = related_model.objects.in_bulk()
                df[field_name] = df[field_name].map(lambda x: f"ID={x}:{related_objects[x]}" if x in related_objects else x)
        ...
        with pd.ExcelWriter(response, engine='openpyxl') as writer:
            ...
            instructions = pd.DataFrame([
                ...
                ["8. File Excel này được tạo lúc: " + current_time + "Tải lại file nếu có dữ liệu mới hơn trên server"],
            ...
            for col_num, column in enumerate(df.columns, 1):
                ...
                cell.fill = PatternFill(start_color="FFFFD4", end_color="FFFFD4", fill_type="solid")
...
def upload_excel(request, model_name):
    ...
    if not request.user.is_superuser:
        return HttpResponse(render_message(request, "Chỉ có admin mới có quyền tải lên file excel", message_type='red'))
    ...
    try:
        ...
        if not excel_file:
            return HttpResponse(render_message(request, "Không phải file excel", message_type='red'))
        ...
        try:
            ...
            return HttpResponse(render_message(request, "File excel không có sheet 'Data'", message_type='red'))
        
        if df.empty:
            return HttpResponse(render_message(request, "Sheet 'Data' không có dữ liệu", message_type='red'))
        ...
        if undefined_columns:
            return HttpResponse(render_message(
                request, 
        ...
        for field_name in fields:
            ...
            if hasattr(field, 'choices') and field.choices:
                if field_name in df_renamed.columns:
                    df_renamed[field_name] = df_renamed[field_name].map(lambda x: choice_mappings[field_name].get(x, x))
            elif field.is_relation and field.many_to_one and field_name in df_renamed.columns:
                ...
                df_renamed[field_name] = df_renamed[field_name].map(get_foreign_key_instance)
            ...
            try:
                ...
                if pd.isna(record_id):
                    ...
                    try:
                        ...
                        if project_id:
                            instance = model_class.objects.get(pk=record_id, project_id=project_id)
                        else:
                        ...
                        if project_id:
                            errors.append(f"Dòng {index + 2}: Không tìm thấy dữ liệu với ID = {int(record_id)} trong dự án này")
                        else:
                            errors.append(f"Dòng {index + 2}: Không tìm thấy dữ liệu với ID = {int(record_id)}")
            except Exception as e:
        ...
        if errors:
            error_message = "\n".join(errors)
            return HttpResponse(render_message(request, error_message, message_type='red'))
        ...
        html_message = render_message(request, message, message_type='green', ok_button_function="reload")
