...
def get_thumbnail(image_url):
    if Thumbnail.objects.filter(reference_url=image_url).exists():
        try:
            thumbnail_url = (
                Thumbnail.objects.filter(reference_url=image_url).first().thumbnail.url
            )
        except Exception as e:
            thumbnail_url = str(e)
...
def calculate_total_operation_time(vehicle_operation_records, gps_name):
    ...
    total_vehicle_time_seconds = vehicle_operation_records.aggregate(
        models.Sum("duration_seconds")
    )["duration_seconds__sum"]
    # calculate total time which has driver
    total_driver_time_seconds = vehicle_operation_records.filter(
        driver__isnull=False
    ).aggregate(models.Sum("duration_seconds"))["duration_seconds__sum"]

    # get the list of driver
    drivers = (
        vehicle_operation_records.filter(driver__isnull=False)
    ...
    data = {
        ...
        "total_driver_time": format_time(
            total_driver_time_seconds - unallowed_revenue_overtime
        ),
        "total_vehicle_time_seconds": total_vehicle_time_seconds,
        "total_driver_time_seconds": total_driver_time_seconds
        - unallowed_revenue_overtime,
        "gps_name": gps_name,
...
def calculate_driver_salary(
    vehicle_operation_records, driver_name, select_start_date=None, select_end_date=None
):
    if not vehicle_operation_records:
        return {
            "success": "false",
    ...
    def get_driver_salary_inputs(driver):
        ...
        if start_time.month == 12:
            ...
            end_date_of_month = datetime(
                start_time.year, start_time.month + 1, 1
            ) - timedelta(days=1)

        driver_salary_inputs = DriverSalaryInputs.objects.filter(
            driver=driver, valid_from__lte=end_date_of_month
        )
        # get the latest
    ...
    def calculate_monthly_salary(
        records, driver_salary_input, select_start_date=None, select_end_date=None
    ):
        # print debug records
        if not records:
            return {}
        ...
        if driver_salary_input.calculation_method == "type_1":
            total_normal_working_days_salary = (
                ...
                * (
                    count_normal_working_days
                    / (count_days_of_month - count_sundays_of_month)
                )
            )

            total_sunday_working_days_salary = (
                driver_salary_input.basic_month_salary
                * driver_salary_input.sunday_month_salary_percentage
                * (count_sunday_working_days / count_days_of_month)
            )
            total_holiday_working_days_salary = (
                driver_salary_input.basic_month_salary
                * driver_salary_input.holiday_month_salary_percentage
    ...
    def calculate_fuel_allowance(records):
        try:
            fuel_allowance = records.aggregate(models.Sum("fuel_allowance"))[
                "fuel_allowance__sum"
            ]
    ...
    def calculate_hourly_salary(records, driver_salary_input):
        ...
        total_normal_working_hours_salary = (
            total_normal_working_hours * driver_salary_input.normal_hourly_salary
        )
        total_sunday_working_hours_salary = (
            total_sunday_working_hours * driver_salary_input.sunday_hourly_salary
        )
        total_holiday_working_hours_salary = (
            total_holiday_working_hours * driver_salary_input.holiday_hourly_salary
        )
        total_overtime_normal_working_hours_salary = (
            total_overtime_normal_working_hours
            * driver_salary_input.normal_overtime_hourly_salary
        )
        total_overtime_sunday_working_hours_salary = (
            total_overtime_sunday_working_hours
            * driver_salary_input.sunday_overtime_hourly_salary
        )
        total_overtime_holiday_working_hours_salary = (
            total_overtime_holiday_working_hours
            * driver_salary_input.holiday_overtime_hourly_salary
        )
        ...
        result = {
            ...
            "total_overtime_normal_working_hours": round(
                total_overtime_normal_working_hours, 3
            ),
            "total_overtime_sunday_working_hours": round(
                total_overtime_sunday_working_hours, 3
            ),
            "total_overtime_holiday_working_hours": round(
                total_overtime_holiday_working_hours, 3
            ),
            "total_normal_working_hours_salary": round(
                total_normal_working_hours * driver_salary_input.normal_hourly_salary, 3
            ),
            "-------": "-----",
    ...
    data_hourly_salary = calculate_hourly_salary(
        vehicle_operation_records, driver_salary_input
    )
    data_monthly_salary = calculate_monthly_salary(
        vehicle_operation_records,
        driver_salary_input,
        select_start_date,
        select_end_date,
    )
    ...
    total_salary = (
        data_monthly_salary["total_monthly_salary"]
...
def calculate_revenue_report(
    vehicle_operation_records, vehicle, select_start_date, select_end_date, update=False
):
    def format_time(time_seconds):
        if time_seconds == None:
            return "00:00:00"
    ...
    if update:
        ...
        unique_start_dates = vehicle_records.values_list(
        ...
        vehicle_revenue_inputs_record = VehicleRevenueInputs.objects.filter(
            vehicle_type=vehicle_instance.vehicle_type
        )
        for input_record in vehicle_revenue_inputs_record:
            revenue_base_display += (
                "- "
                + input_record.valid_from.strftime("%d/%m/%Y")
                + ": "
                + str(format_money_PL(input_record.revenue_day_price))
                + " VND - "
                + str(input_record.number_of_hours)
                + " giờ"
                + "\n"
            )
        ...
        for start_date in unique_start_dates:
            ...
            vehicle_records_with_driver_for_date = vehicle_records_with_driver.filter(
                start_time__date=start_date
            )
            # calculate total time which has driver
            working_time_seconds = vehicle_records_with_driver_for_date.aggregate(
                models.Sum("duration_seconds")
            )["duration_seconds__sum"]
            unallowed_revenue_overtime = 0
            ...
            working_time_hours = (
                working_time_seconds - unallowed_revenue_overtime
            ) / 3600
            total_working_hours += working_time_hours

            date_vehicle_revenue_inputs_record = VehicleRevenueInputs.get_valid_record(
                vehicle_instance.vehicle_type, start_date
            )
            if not date_vehicle_revenue_inputs_record:
                revenue = (
                    "Không có dữ liệu tính doanh thu ngày  "
            ...
            date_revenue = (
                date_vehicle_revenue_inputs_record.revenue_day_price
                / date_vehicle_revenue_inputs_record.number_of_hours
            ) * working_time_hours
            revenue += date_revenue

        # calculate fuel cost
        filling_records = FillingRecord.objects.filter(
            vehicle=vehicle_instance,
            fill_date__gte=min_start_date,
            fill_date__lte=max_end_date,
        )

        if filling_records:
            fuel_filling_cost_amount = filling_records.filter(
                liquid_type__in=["diesel", "gasoline"]
            ).aggregate(models.Sum("total_amount"))["total_amount__sum"]
            other_filling_cost_amount = filling_records.exclude(
                liquid_type__in=["diesel", "gasoline"]
            ).aggregate(models.Sum("total_amount"))["total_amount__sum"]
        ...
        for n in range(int((max_end_date - min_start_date).days) + 1):
            ...
            vehicle_depreciation_record = VehicleDepreciation.get_vehicle_depreciation(
                vehicle_instance, d
            )
            ...
            bank_interest_record = VehicleBankInterest.get_vehicle_bank_interest(
                vehicle_instance, d
            )
        ...
        maintenance_amount = VehicleMaintenanceRepairPart.get_maintenance_amount(
            vehicle_instance, min_start_date, max_end_date
        )

        total_cost = (
            fuel_filling_cost_amount
            + other_filling_cost_amount
            + maintenance_amount
            + depreciation_amount
        ...
        for driver in drivers:
            ...
            vehicle_records_per_driver = vehicle_records_with_driver.filter(
                driver=driver
            )
            salary_data = calculate_driver_salary(
                vehicle_records_per_driver, None, select_start_date, select_end_date
            )
            if salary_data["success"] == "false":
                ...
                monthly_salary = salary_data["data"]["monthly_salary"][
                    "total_monthly_salary"
                ]
                hourly_salary = salary_data["data"]["hourly_salary"][
                    "total_hourly_salary"
                ]
                fixed_allowance = salary_data["data"]["total_fixed_allowance"]
                fuel_allowance = salary_data["data"]["fuel_allowance"]
                total_cost += (
                    monthly_salary + hourly_salary + fixed_allowance + fuel_allowance
                )
                ...
                hourly_salary_display += (
                    f"- {driver.full_name}: {format_money_PL(hourly_salary)}\n"
                )
        ...
        vehicles = VehicleDetail.objects.filter(
            vehicle_type__allowed_to_display_in_revenue_table="Cho phép"
        )
        ...
        unique_gps_vehicles = [
            value for value in unique_gps_vehicles if not value.startswith("72")
        ]
...
@register.filter(name="get_sorts")
def get_sorts(model):
    model_class = globals()[model]
    ...
    fields = []
    for field in model_class._meta.fields:
        # Bỏ qua các field từ BaseModel
        if field.name in ["last_saved", "archived", "lock", "created_at", "updated_at"]:
            continue

        # Lấy tên hiển thị của field (verbose_name)
        display_name = field.verbose_name

        # Thêm vào danh sách các tùy chọn sắp xếp
        if field.get_internal_type() in ["DateField", "DateTimeField"]:
            # Đối với các trường ngày tháng, tạo tùy chọn gần nhất/xa nhất
            fields.append(
                {
                    "sort": f"-{field.name}",
                    "display_name": f"{display_name} - (gần nhất trước)",
                }
            )
            fields.append(
                {
                    "sort": field.name,
                    "display_name": f"{display_name} - (xa nhất trước)",
                }
            )
        else:
            # Đối với các trường khác, tạo tùy chọn A-Z/Z-A
            fields.append(
                {"sort": field.name, "display_name": f"{display_name} - (A → Z)"}
            )
            fields.append(
                {"sort": f"-{field.name}", "display_name": f"{display_name} - (Z → A)"}
            )
